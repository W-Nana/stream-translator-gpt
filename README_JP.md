...
WebUI のコマンドログは、表示前に API キー、Bot トークン、Webhook URL、認証情報付きプロキシ URL をマスクします。字幕共有の読み取りエンドポイントは有効化中は公開です。`0.0.0.0` に bind すると、LAN 内の他デバイスから現在の字幕を読めます。
...
| `--insecure_api_tls`                    |                                | API クライアントとモデルダウンロードの TLS 証明書検証を無効化します。信頼済みのデバッグプロキシまたは管理されたネットワークでのみ使用してください。                  |
| **入力オプション**                      |                                |                                                                                                                                                                       |
